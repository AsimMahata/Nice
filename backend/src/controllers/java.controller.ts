...
const codeDir = path.join(__dirname, "../../temp/java/code");
const resDir = path.join(__dirname, "../../temp/java/res");
const source = path.join(codeDir, "Main.java"); 
...
const compile = (code: string): Promise<Status> => {
  ...
  return new Promise((resolve) => {
    exec(`javac "${source}" -d "${resDir}"`, (err, _, stderr) => {
      if (err) {
        status.error = stderr;
...
export const runCode = async (req: Request, res: Response) => {
  ...
  const child = spawn("java", ["-cp", resDir, "Main"], { stdio: "pipe" });
