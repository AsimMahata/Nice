...
export function useFileActions({ showToast, setCode }: props) {
    ...
    const [currentDirPath, setCurrentPath] = useState<string | null>(null);
    ...
    async function loadFiles() {
        ...
        setFiles(
            entries.map(e => ({
                ...
                isDir: e.isDirectory,
                dir: currentDirPath,
                filePath: currentDirPath + '/' + e.name
            }))
        );
        console.log('frontend file action ts reading Directory', entries)
    }
    // on click what to do
    ...
    async function goParentDir() {
        try {
            const parent = await dirname(currentDirPath || "ERROR");
    ...
    async function createNewFolder(folderName: string) {
        ...
        const location = `${currentDirPath}/${folderName}`;
    ...
    async function createNewFiles(fileName: string) {
        ...
        const location = `${currentDirPath}/${fileName}`;
